...
class ScreenRecorder(QWidget):
    ...
    def update_screen_selection(self):
        """更新選擇的螢幕"""
        selected_screen_name = self.screen_selector.currentText()
        self.selected_screen_geometry = self.screen_list[selected_screen_name]
...
class ScreenSelection(QWidget):
    ...
    def mouseReleaseEvent(self, event):
        ...
        p = 5  # 防止框線被錄進去的擴大範圍
        x, y, w, h = int(x * scaling) - p, int(y * scaling) - p, int(w * scaling) + (2 * p), int(h * scaling) + (2 * p)
